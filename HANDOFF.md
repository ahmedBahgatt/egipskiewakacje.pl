...
- **Headline price wording (deployed):** booking card + sticky bar drop "od"/"/ os."; show base plainly (`60 USD`) + data-driven caption `priceHeadline()` in `lib/format.ts` - per-person → "za osobę dorosłą" (compact "dorosły" ≤360px), other modes → "za {unit}" (za łódź/za buggy). Breakdown rows, pricing data, booking logic, SEO/schema unchanged. Listing `TourCard`/`TourSlider` still use `priceLabel`/`priceUnit` (out of scope, unchanged).
